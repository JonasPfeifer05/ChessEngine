...
public class Game {
	...
	public Player getNextPlayer(int index) {
		index++;
		index %= playerCount;
		if (!isAlive(Player.values()[index])) return getNextPlayer(index);
    ...
    public void killUnableToMoveNextPlayer() {
		if (lookStaleMate(getNextPlayer(currentPlayerIndex)) || lookCheckMate(getNextPlayer(currentPlayerIndex))) setAlive(getNextPlayer(currentPlayerIndex), false);
    }
